import { afterEach, describe, expect, it, mock } from 'bun:test'
import { initModelProvider, resolveModel } from '../src/config/models.js'

const envKeys = [
  'LLM_PROXY_URL',
  'EIGEN_GATEWAY_URL',
  'LLM_PROXY_API_KEY',
  'KMS_AUTH_JWT',
  'KMS_SERVER_URL',
  'KMS_PUBLIC_KEY',
] as const

const savedEnv: Record<string, string | undefined> = {}
for (const key of envKeys) {
  savedEnv[key] = process.env[key]
}

afterEach(() => {
  for (const key of envKeys) {
    if (savedEnv[key] === undefined) {
      delete process.env[key]
    } else {
      process.env[key] = savedEnv[key]
    }
  }
})

describe('proxy-backed model resolution', () => {
  it('fails fast when the proxy base URL is not configured', async () => {
    delete process.env.LLM_PROXY_URL
    delete process.env.EIGEN_GATEWAY_URL
    process.env.LLM_PROXY_API_KEY = 'test-key'

    await expect(initModelProvider()).rejects.toThrow('Gateway URL is required')
  })

  it('fails fast when proxy auth is not configured', async () => {
    process.env.EIGEN_GATEWAY_URL = 'https://proxy.example.com'
    delete process.env.LLM_PROXY_API_KEY
    delete process.env.KMS_AUTH_JWT
    delete process.env.KMS_SERVER_URL
    delete process.env.KMS_PUBLIC_KEY

    await expect(initModelProvider()).rejects.toThrow('LLM_PROXY_API_KEY or KMS_AUTH_JWT is required')
  })

  it('accepts attestation config as an alternative to a static JWT', async () => {
    process.env.EIGEN_GATEWAY_URL = 'https://proxy.example.com'
    delete process.env.LLM_PROXY_API_KEY
    delete process.env.KMS_AUTH_JWT
    process.env.KMS_SERVER_URL = 'https://kms.example.com'
    process.env.KMS_PUBLIC_KEY = 'test-public-key'

    await expect(initModelProvider()).resolves.toBeUndefined()
  })

  it('resolves models through the proxy provider when configured', async () => {
    process.env.EIGEN_GATEWAY_URL = 'https://proxy.example.com'
    process.env.LLM_PROXY_API_KEY = 'test-key'

    await initModelProvider()
    const model = resolveModel({ engagement: 'anthropic/claude-haiku-4.5' } as any, 'engagement')
    expect(model).toBeTruthy()
  })
})
