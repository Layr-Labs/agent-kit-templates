...
/**
 * ArXiv Reader — search and read full papers from ArXiv.
 *
 * Uses the ArXiv API for search/metadata and fetches HTML versions
 * of papers for full-text extraction (no PDF parsing needed).
 */
const skill = {
  name: 'arxiv-reader',
    ...
    return {
        ...
        execute: async ({ query, categories, max_results }) => {
          const params = new URLSearchParams({
            search_query: categories
              ? `all:${query} AND (${categories.split(',').map(c => `cat:${c.trim()}`).join(' OR ')})`
              : `all:${query}`,
            start: '0',
      ...
      get_paper_metadata: tool({
        ...
        execute: async ({ arxiv_id }) => {
          ...
          if (results.length === 0) throw new Error(`Paper not found: ${arxiv_id}`)
          return results[0]
        },
      }),
      ...
      read_paper: tool({
        ...
        execute: async ({ arxiv_id }) => {
          ...
          // Try HTML version first (best for extraction)
          ...
          if (!res.ok) {
            // Fallback: try the abstract page
            const absRes = await fetch(`https://arxiv.org/abs/${cleanId}`, {
              headers: { 'User-Agent': 'Mozilla/5.0 (compatible; ArXivReader/1.0)' },
            })
            if (!absRes.ok) throw new Error(`Could not fetch paper ${arxiv_id}: ${absRes.status}`)
...
function parseArxivSearch(xml: string): Array<{
  id: string
  title: string
  authors: string[]
  abstract: string
...
function extractHtmlPaper(html: string): string {
  ...
  const mainMatch = text.match(/<main[\s\S]*?<\/main>/i)
    ...
    ?? text.match(/<div class="ltx_page_content"[\s\S]*?<\/div>\s*<\/div>/i)
  ...
  // Truncate if extremely long (some papers are huge)
...
function extractAbstractPage(html: string): string {
