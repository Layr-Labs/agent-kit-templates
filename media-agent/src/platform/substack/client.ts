...
export class SubstackClient {
  ...
  /**
   * Check if we're currently logged into Substack by testing the API.
   */
  async isLoggedIn(): Promise<boolean> {
    const browser = this.browser as any
    try {
      // Navigate to a Substack page if not already there
      ...
      const result = await browser.evaluate(`
        (async () => {
          try {
            ...
            return r.ok ? 'logged_in' : 'not_logged_in';
          } catch { return 'error'; }
        })()
      `)
...
Enter email: ${agentEmail}
Click continue.
Then use the wait_for_email tool to get the verification email. Do NOT navigate to any email website.
The email will contain either a magic link URL or a 6-digit code.
- If magic link: navigate to that URL
- If 6-digit code: type it into the verification field

After logging in, verify you're on the dashboard.

IMPORTANT:
- If you see "Too many login emails", wait 60 seconds and try again (only retry ONCE).
- Do NOT search the filesystem for credentials.
- Do NOT navigate to eigenagents.org or any email provider.
- Return "logged_in" if successful, or "login_failed: <reason>" if not.`,
