...
export class TwitterClient {
  ...
  constructor(private events: EventBus, readProvider: TwitterReadProvider, private config: Config) {
  ...
  async postCartoon(opts: {
    ...
    if (!this.config.twitter.postingEnabled) {
      ...
      this.events.emit({
        ...
        text: opts.text,
        imageUrl: toCdnUrl(opts.imagePath, 'images', this.config.r2),
        ts: Date.now(),
      })
  ...
  async postVideo(opts: {
    ...
    if (!this.config.twitter.postingEnabled) {
      ...
      this.events.emit({
        ...
        platformId: `local-${localId}`,
        text: opts.text,
        imageUrl: toCdnUrl(opts.videoPath, 'videos', this.config.r2),
        ts: Date.now(),
      })
